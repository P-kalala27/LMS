...
const AddCourse = () => {
  ...
  const [chapters, setChaptres] = useState([])
  ...
  const handleChapter =  (action, chapterId) => {
    ...
    if(action === 'add'){
      ...
      if(title){
        ...
        setChaptres([...chapters, newChapter]);
      }
    } else if (action === 'remove') {
      setChaptres(chapters.filter((chapter) => chapter.chapterId !== chapterId))
  ...
  const handlerLectur =  (action, chapterId, lectureIndex) => {
    if(action === 'add'){
  ...
  return (
    ...
    justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      <form onSubmit={handleSubmite}  className="flex flex-col gap-4 max-w-md w-full text-gray-500">
        <div className='flex flex-col gap-1'>
          <p>Course Title</p>
           <input
            ...
            className="outiline-none md:py-2.5 py-2 px-3 rounded border border-gray-500"
            required
          />
        </div>
        <div className="flex flex-col gap-1">
        ...
        <div>
                  ...
                  <div className="p-4">
                    {
                      chapter.chapterContent.map((lecture, lectureIndex) => (
                        <div className='flex justify-between items-center mb-2' key={lectureIndex}>
                          ...
                          alt=""
                          className="cursor-pointer"
                          onClick={() => handlerLectur('remove', chapter.chapterId, lectureIndex)}/>
                        </div>
                      ))
                    }
                    <div className='inline-flex bg-gray-100 p-2 rounded cursor-pointer mt-2 '
                    onClick={() => handlerLectur('add', chapter.chapterId)}>+ Add Lecture</div>
                  </div>
                )
              }
             </div> 
         ...
         <button type='button' className="bg-black text-white w-max py-2.5
          px-8 rounded my-4 cursor-pointer"
          onClick={addLecture}>
          ADD
        </button>
      </form>
